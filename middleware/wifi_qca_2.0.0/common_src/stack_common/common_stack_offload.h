...
#define SOCK_EV_MASK_SET(_pcxt, _cmd) (_pcxt)->sock_st_mask[(_cmd) >> 5] |= (uint32_t)1 << ((_cmd)&0x1f)

#define SOCK_EV_MASK_CLEAR(_pcxt, _cmd) (_pcxt)->sock_st_mask[(_cmd) >> 5] &= ~(1 << ((_cmd)&0x1f))

#define SOCK_EV_MASK_TEST(_pcxt, _cmd) (((_pcxt)->sock_st_mask[(_cmd) >> 5] & (uint32_t)(1 << ((_cmd)&0x1f))) ? 1 : 0)
